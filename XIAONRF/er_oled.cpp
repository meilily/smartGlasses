...
void customChar(unsigned char x, unsigned char y, char acsii, char mode, uint8_t* buffer)
{
    char aSize = 5; // size of array for each ascii char byte array
    unsigned char i, j, y0=y;
    char temp;
    unsigned char ch = acsii - ' ';
    for(i = 0;i<aSize;i++) {    
        temp = pgm_read_byte(&test88[ch][i]);
        for(j =0;j<8;j++)
        {
            if(temp & 0x80) er_oled_pixel(x, y, 1, buffer);
            else er_oled_pixel(x, y, 0, buffer);
...
void customString(uint8_t x, uint8_t y, const char *pString, uint8_t Mode, uint8_t* buffer)
{
    int size;
    while (*pString != '\0') {      
        const char curChar = *pString;
        if(curChar == 'M' || curChar == 'W' || curChar == 'O'){
          size = 6;
        }
        else if(curChar == '1' || curChar == 'I'||curChar == 'T' || curChar == 'V'||curChar =='Y'){
          size = 4;
        }
        else if(curChar ==':'){
          size = 2;
        ...
        customChar(x, y, *pString, Mode, buffer);
        x += size;
        pString++;
    }
}
